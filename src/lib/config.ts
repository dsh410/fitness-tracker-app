...
export function introCopy(age: number | null = AGE): string {
  ...
  return `${who} I started at ${START_WEIGHT} pounds with type 2 diabetes. I'm logging every weigh-in here, publicly, so I can't quietly quit. No program, no advice — just the numbers.`;
}
